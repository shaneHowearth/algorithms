...
import (
	...
	orderedlinear "github.com/shanehowearth/algorithms/search/ordered_linear"
...
func Test(t *testing.T) {
	...
	for name, tc := range testcases {
		t.Run(name, func(t *testing.T) {
			found, err := orderedlinear.Search(tc.find, tc.input)
			if tc.err != nil {
...
func BenchmarkSearch(b *testing.B) {
	...
	find := rand.Intn(b.N)
	for i := 0; i < b.N; i++ {
		orderedlinear.Search(find, input)
	}
}
